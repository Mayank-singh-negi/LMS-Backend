...
export const sendOTPEmail = async (to, otp) => {
  ...
  await transporter.sendMail({
    ...
    to,
    subject: "Your Verification Code – Learnovora",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;background:#f9f9f9;border-radius:12px;">
        ...
        <div style="background:#004643;color:#fff;font-size:36px;font-weight:bold;letter-spacing:12px;text-align:center;padding:20px 32px;border-radius:10px;">
          ${otp}
        </div>
...
export const sendCourseApprovedEmail = async (teacherEmail, teacherName, courseTitle) => {
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.log(`\n✅ [DEV] Course approved email → ${teacherEmail} | Course: "${courseTitle}"\n`);
    return;
  }
